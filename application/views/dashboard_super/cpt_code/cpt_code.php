  ...
  <section class="box">
    <table class="table" style="border-bottom: 0.5px solid #c6c4c1;">
      <tbody>
        <tr>
          <td width="15%">
            <a href="<?php echo base_url("dashboard_super/cpt_code/cpt_code/create") ?>">
          ...
          <td width="15%">
            <a href="<?php echo base_url("dashboard_super/cpt_code/cpt_code/upload_cptcode") ?>">
              <button id="new" class="btn btn-default" style="width: 90%;margin-right: 11px;">
                <span class="fa fa-user"></span>
                &nbsp;&nbsp;Upload CPT Code</span>
              </button>
            </a>
          </td>
    ...
    <div class="col-lg-12" style="min-height: 850px;">
      <table class="table table-admin tableBlock">
        ...
        <tbody id="adminadd">
          ...
          if (!empty($cpt_code)) { ?>
                            <?php $sl = 1; ?>
                            <?php foreach ($cpt_code as $value) { ?>
          <tr style="border-bottom: 1px solid #ddd;" class="hovertr">

<td  ><span class="text-primary"><?php echo $sl; ?></span></td>
<td  ><span class="text-primary"><?php echo $value->cpt_code; ?></span></td>
<td><select class="btn btn-default dropdown-toggle" onchange="call('<?php echo $value->cpt_id; ?>',this.options[this.selectedIndex].value)">
...
<td>
  <div class="btn-group" style="float: right;display: flex;">
    <!-- <a href="#" onclick="admin_info('<?php //echo $value->allergy_id; ?>')" class="btn btn-xs btn-default" style="margin-right:10px;"><i class="fa fa-eye"></i></a> -->
                      <a  href="<?php echo base_url("dashboard_super/cpt_code/cpt_code/edit/$value->cpt_id") ?>" class="btn btn-xs btn-default" style="margin-right:10px;"><i class="fa fa-edit"></i></a>
                      <a href="<?php echo base_url("dashboard_super/cpt_code/cpt_code/delete/$value->cpt_id") ?>" class="btn btn-xs btn-danger" onclick="return confirm('<?php echo display('are_you_sure') ?> ') "> <i class="fa fa-trash"></i></a>
                    </div>
  <!-- <a href="#" onclick="admin_info('<?php //echo $value->allergy_id; ?>')" class="btn btn-xs btn-default" style="margin-right:10px;"><i class="fa fa-eye"></i></a> -->
...
function call(id,value)
{
 //alert(value);
  ...
  if(changeStatus){
      $.ajax({
           url:'<?=base_url()?>dashboard_super/cpt_code/cpt_code/changestatus',
           ...
           success: function(msg){
             ...
             $("#h").text("CPT code status has been "+v);
          //   alert(msg);
    $('#active').modal('toggle');
            // alert(msg);
   ...
   setTimeout(function() {
        ...
        window.location="<?=base_url()?>dashboard_super/cpt_code/cpt_code";
    }, 3000);
}
