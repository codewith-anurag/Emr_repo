...
class Doctor extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model(array(
			...
			'dashboard_doctor/doctor/department_model'
		));
...
<table class="table" style="width: 100%;">
 ...
 <tr>
    ...
    <td width="60%" style="border-right: 1px solid #d7d0d0;border-left: 1px solid #d7d0d0;background-color: white;">
      <p style="color: #150aec;font-weight: 700;font-size: 16px;">New register in GT Health System</p>
      <p><b>Hello '.$this->input->post('email',true).',</b></p>
      <p>Your account has been registered on GT Health System</p>
      ...
      <td width="60%" style="border-right: 1px solid #d7d0d0;border-left: 1px solid #d7d0d0;background-color: white;">
        <table class="table" style="border-collapse: collapse;margin-left: 15px;margin-right: 15px;margin-top:15px;width: 90%;">
          <tbody>
			...
			<p style="color: #6f5f5f;">* Thanks! For new register in GT Health System application.</p>
		</td>
	<td width="20%"></td>
</tr>
<tr>
 <td width="20%"></td>
	...
	<td width="20%"></td>
					...
					redirect('dashboard_doctor/doctor/doctor/create');
				}
				...
				redirect('dashboard_doctor/doctor/doctor/');
			} else {
				if ($this->doctor_model->update($postData)) {
					...
					#set exception message
					$this->session->set_flashdata('exception', display('please_try_again'));
					redirect('dashboard_doctor/doctor/doctor/edit/'.$postData['user_id']);
				}
			...
			$data['content'] = $this->load->view('dashboard_doctor/doctor/doctor_form',$data,true);
			$this->load->view('dashboard_doctor/main_wrapper',$data);
		}
	}
