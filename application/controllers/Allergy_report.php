...
class Allergy_report extends CI_Controller {
	...
	public function index()
	{
		...
		$drugallergy  = $this->db->get_where("allergy_type",array("status"=>1))->result();

		$data['drugallergy'] = $drugallergy;   
		$data['title'] = display('Allergy Report');
		$data['patients'] = $this->patient_model->read();
		$data['content'] = $this->load->view('allergy_report',$data,true);
		$this->load->view('layout/main_wrapper',$data);
	}
	
	
	function allergy_report_search($patient_id=""){

		$allergy 	= $this->input->get_post('allergy');
		$drug_allergy 	= $this->input->get_post('drug_allergy');
		
		$sql = "";
	   		if($allergy !="" && $drug_allergy!="" ){

				$sql ="SELECT * FROM healthreport_allergies		 
			  	WHERE  healthreport_allergies.allergy_type = '".$allergy."' and drug_allergy = '".$drug_allergy."'  ORDER BY date DESC";
			}else if($allergy!=""){

				$sql ="SELECT * FROM healthreport_allergies		 
			  	WHERE  healthreport_allergies.allergy_type = '".$allergy."'  ORDER BY date DESC";
			}else if($drug_allergy!=""){

				$sql ="SELECT * FROM healthreport_allergies		 
			  	WHERE   drug_allergy = '".$drug_allergy."'  ORDER BY date DESC";
			}
			...
			$searchdetail =  $query->result();
			 ...
			 if(count($searchdetail)>0){
				 foreach ($searchdetail as $value) {
					 ...
					 $msg.='<td><div class="kpull-left"><div class="word-break">';
					 $msg.='<span class="text-primary">'.$value->allergy_type.' : '.$value->drug_allergy.'</span>';
					 $msg.='</div></div></td>';
					 $msg.='<td>'.$value->reaction.'</td>';					 
	...
	public function download_excel()
	{
		...
		$objPHPExcel->getActiveSheet()->setCellValue('A1', 'Allergy Report');
		...
		$objPHPExcel->getActiveSheet()->setCellValue('C2', 'Allergy');
		$objPHPExcel->getActiveSheet()->setCellValue('D2', 'Reaction');		
		...
		$row = 3;
		$allergy 		= !empty($_GET['allergy']) ? $_GET['allergy'] : "";
		$drug_allergy 	= !empty($_GET['drugallergy']) ? $_GET['drugallergy'] : ""	;

		$sql = "";
	   	if($allergy !="" && $drug_allergy!="" ){

			$sql ="SELECT * FROM healthreport_allergies	WHERE  healthreport_allergies.allergy_type = '".$allergy."' and drug_allergy = '".$drug_allergy."'  ORDER BY date DESC";
		}else if($allergy!=""){

				$sql ="SELECT * FROM healthreport_allergies	WHERE  healthreport_allergies.allergy_type = '".$allergy."'  ORDER BY date DESC";
		}else if($drug_allergy!=""){
				$sql ="SELECT * FROM healthreport_allergies	WHERE   drug_allergy = '".$drug_allergy."'  ORDER BY date DESC";
		}
		$query = $this->db->query($sql);
		$searchdetail =  $query->result();
		$msg ='';
		if(count($searchdetail)>0){
			foreach ($searchdetail as $value) {
				...
				$objPHPExcel->getActiveSheet()->setCellValue('C'.$row, $value->allergy_type.':'.$value->drug_allergy);
				$objPHPExcel->getActiveSheet()->setCellValue('D'.$row, $value->reaction);				
			...
			header('Content-Disposition: attachment;filename="Allergy_Report.xls"');
...
redirect('patient');
	...
	function allergy_report_pdf()
	{
		...
		$pdfname = "Allergy_report" . date('YmdHis') . '.pdf';
		$html = '<style>
								table {
            			...
            			<h3 style="text-align:center;">Allergy Report</h3>
			...
			$sql ="SELECT * FROM healthreport_allergies		 
				...
				$query = $this->db->query($sql);
				$allergya = $query->result_array();
			foreach ($allergya as $allergy_list) { 
					$sqlQ = " select patient.id,fname,lname,prefix,date_of_birth from patient where id=".$allergy_list['patient_id'];
		 			$Pquery = $this->db->query($sqlQ);
					...
					$html.='<tr style="border-bottom: 1px solid #ddd;" class="hovertr">';
					 ...
					 $html.='<td><div class="kpull-left"><div class="word-break">';
					 $html.='<span class="text-primary">'.$allergy_list['allergy_type'].':'.$allergy_list['drug_allergy'].'</span>';
					 $html.='</div></div></td>';


					 $html.='<td>'.$allergy_list['reaction'].'</td>';
					 $html.='<td>'.$allergy_list['status'].'</td>
					</tr>';
