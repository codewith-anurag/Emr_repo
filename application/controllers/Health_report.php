...
<a href="'.base_url().'/health_report/recordvaccination_print/'.$value->id.'" target="_blank" class="btn btn-xs icon-box"><i class="fa fa-print"></i></a>
	...
	public function edit_vitalsing()
	{
        ...
        $query = $this->db->query("select patient_id from pa_vital_sign where patient_id='$patient_ID' and id='$id'");
        $no    = $query->num_rows();
        $result = $query->row();
        $healthrecord_id = $result->healthrecord_id;
	...
	function edit_update_allergylist(){
        ...
        $query = $this->db->query("select patient_id from healthreport_allergies where patient_id='$patient_ID' and id='$allergies_id'");
        $no    = $query->num_rows();
        if ($no == 1) {
	...
	public function edit_labresult()
	{
        ...
        $query = $this->db->query("select patient_id from lab_result where patient_id='$patient_ID' and loinc_code_id='$healthreport_labresult_id'");
        $no    = $query->num_rows();
        if ($no == 1) {
	...
	public function edit_imagingorder()
	{
		...
		$doctor_comments	= $this->input->post("doctor_comments");
        $session_healthrecord_id = $this->session->userdata("edit_healthrecord_ids");
        $sql = "select patient_id from imaging_order where patient_id='$patient_ID' and imaging_order_id='$healthreport_imaging_id'";
...
public function edit_e_prescription()
{
    ...
    $query = $this->db->query("select patient_id from e_prescription where patient_id='$patient_ID' and e_prescription_id='$e_prescription'");
    //echo $this->db->last_query();exit;
    $no    = $query->num_rows();
    if ($no == 1) {
	...
	public function edit_summary()
	{
        ...
        $query = $this->db->query("select patient_id from healthreport_summary where patient_id='$patient_ID' and summary_id='$summary_id'");            
        if (!empty($query)) {
    ...
    public function edit_Vaccines()
    {
        ...
        $query = $this->db->query("select patient_id from healthreport_lockedclinicalnotes where patient_id='$patient_ID' and id='$vaccines_e_ID'");
        $no  = $query->num_rows();   
        if ($no == 1) {
	...
	public function edit_clinical_notes(){
		...
		//print_r($clinical_notes);exit;
        $sql = "select patient_id from healthreport_lockedclinicalnotes where patient_id='$patient_ID' and healthrecord_id='$healthrecord_id'";
	...
	public function edit_singed_consent(){
		...
		$sql = "select patient_id from healthreport_singedconsentforms where patient_id='$patient_ID' and healthrecord_id='$healthrecord_id'";
        $ex = $this->db->query($sql);  
        $row = $ex->row();     
        $num    = $ex->num_rows();   
        if(empty($row)){
	...
	public function edit_lab_result(){
		...
		$lab_result = array("patient_id"=>$patient_ID,"lab"=>$lab,"date"=>$date,"action"=>$action,"abnormal"=>$abnormal ,"result_count"=>$result_count,"test"=>$test,"date"=>date("Y-m-d H:i:s"));
         $sql = "select patient_id from document_labresult where patient_id='$patient_ID' and healthrecord_id='$healthrecord_id'";
        $ex = $this->db->query($sql);  
        $row = $ex->row();     
        $num    = $ex->num_rows();   
        if($num == 1){
	...
	public function edit_document_form() {
        ...
        $query = $this->db->query("select patient_id from healthreport_document where patient_id='$patient_ID' and id='$healthreport_document_id' and document_type='$document_type'");
        $no    = $query->num_rows();              
        if($no == 1){            
