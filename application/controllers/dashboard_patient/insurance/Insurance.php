...
class Insurance extends CI_Controller {
	...
	//	$data['patients'] = $this->patient_model->read();
		$data['insurance'] = $this->db->select("*")->from("insurance")->where('patient_id',$patient_id)->get()->result();   //$this->insurance_model->read();
		$data['content'] = $this->load->view('dashboard_patient/insurance/insurance',$data,true);
...
$hospital_id = $this->db->select("*")->from("patient")->where("id",$this->session->userdata('user_id'))->get()->row();
//$hospital_id  =
			#-------------------------------#
...
redirect('dashboard_patient/insurance/insurance');
				...
				if ($this->insurance_model->update($postData)) {
					...
					#set exception message
					$this->session->set_flashdata('exception', display('please_try_again'));
					redirect('dashboard_patient/insurance/insurance/edit/'.$postData['id']);
				}
			...
			$data['content'] = $this->load->view('dashboard_patient/insurance/insurance_form',$data,true);
			$this->load->view('dashboard_patient/main_wrapper',$data);
		}
	}
