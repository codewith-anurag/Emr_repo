...
class Medicalprovider_report extends CI_Controller {
	...
	public function index()
	{
		...
		$sql ="SELECT * FROM user WHERE  hospital_id = $id  and user_role='2' and  status='1'   ORDER BY create_date DESC";
		$query = $this->db->query($sql);
		$medicalprovider =  $query->result();
		$data["medicalprovider"] = $medicalprovider;
		$data['title'] = display('Medical Provider Report');
		$data['patients'] = $this->patient_model->read();
		$data['content'] = $this->load->view('medicalprovider_report',$data,true);
		$this->load->view('layout/main_wrapper',$data);
	}
	
	
	function medicalprovider_report_search($patient_id=""){
		...
		if($from_date!="" && $to_date!=""){
			...
			$sql ="SELECT * FROM user		 
		  	WHERE hospital_id ='$id' and user_role='2' and  DATE(user.create_date) >= '$from_date' and DATE(user.create_date)  <= '$to_date' OR (DATE(user.create_date) = '$from_date') ORDER BY create_date DESC";
		}
	   ...
	   if(count($searchdetail)>0){
		 foreach ($searchdetail as $value) {
			 ...
			 $msg.='<span class="text-primary">'.$value->firstname."  ".$value->lastname.'</span>';
			 $msg.='</div></div></td>';

			 $msg.='<td><span class="text-primary">'.$value->email.'</span></td>';
			 $msg.='<td><div class="kpull-left"><div class="word-break">';
			 $msg.='<span class="text-primary">'.$value->phone .'</span>';
			 $msg.='</div></div></td>';


			 $msg.='<td>'.$value->mobile.'</td>';			 
	...
	public function sendEmailAttachment($to, $subject, $htmlMessage) {
		...
		$objPHPExcel->getActiveSheet()->setCellValue('C2', 'Office Phone');
		$objPHPExcel->getActiveSheet()->setCellValue('D2', 'Home Phone');
		...
		if($from_date!="" && $to_date!=""){
			...
			$sql ="SELECT * FROM user WHERE  status='1' and hospital_id ='$id' and user_role='2' ORDER BY create_date DESC";
			$query = $this->db->query($sql);
			$doctor =  $query->result();
		}
		...
		foreach ($doctor as $value) { 
			...
			$objPHPExcel->getActiveSheet()->setCellValue('D'.$row, $value->mobile);
			$objPHPExcel->getActiveSheet()->setCellValue('E'.$row, $value->status);
		...
		header('Content-Disposition: attachment;filename="medicalprovider_report.xls"');
...
'reminder_call' 	   => $this->input->post('reminder_call'),
	...
	{		
		...
		$pdfname = "Medical Provider Report" . date('YmdHis') . '.pdf';
		$html = '<style>
								table {
            			...
            			<h3 style="text-align:center;">Medical Provider Report</h3>
			...
			foreach ($doctor as $doctor_list) { 
					if($doctor_list['status']==1){
						$doctor_list['status']='Active';
					}else  {
						$doctor_list['status']='Inactive';
					}
					$html.='<tr style="border-bottom: 1px solid #ddd;" class="hovertr">';

					 $html.='<td><div class="kpull-left"><div class="word-break">';
					 $html.='<span class="text-primary">'.$doctor_list['firstname']."  ".$doctor_list['lastname'].'</span>';
					 $html.='</div></div></td>';

					 $html.='<td><span class="text-primary">'.$doctor_list['email'].'</span></td>';
					 $html.='<td><div class="kpull-left"><div class="word-break">';
					 $html.='<span class="text-primary">'.$doctor_list['phone'].'</span>';
					 $html.='</div></div></td>';


					 $html.='<td>'.$doctor_list['mobile'].'</td>';					 
					 $html.='<td>'.$doctor_list['status'].'</td>
					</tr>';
