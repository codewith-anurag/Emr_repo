...
class Productivity_report extends CI_Controller {
	...
	{	    $id = $this->session->userdata('user_id');
		...
		$doctor  = $this->db->get_where("user",array("status"=>1,"hospital_id"=>$id,"user_role"=>'2'))->result();			
		$data['doctor'] = $doctor;   
		$data['title'] = display('Productivity Report');
		$data['patients'] = $this->patient_model->read();
		$data['content'] = $this->load->view('productivity_report',$data,true);
		$this->load->view('layout/main_wrapper',$data);
	}
	
	
	function productivity_report_search($patient_id=""){
		$id = $this->session->userdata('user_id');
		    $created_by_id = $this->session->userdata('created_by');
        	$isadmin = $this->session->userdata('isadmin');
	        if($isadmin == 1){
		...
		$sql = "";
   		if($doctor!="" && $from_date!="" && $to_date!="" ){
   			$sql = "SELECT doctor_id,hospital_id,count(schedule_id) as total_apointment,sum(duration) as total_duration FROM schedule WHERE  (hospital_id = '".$id."' and  schedule.doctor_id = '".$doctor."' and DATE(schedule.whens) >= '$from_date' and DATE(schedule.whens)  <= '$to_date' and type='appointment')  group by doctor_id  ORDER BY created_date DESC";
		}else if($doctor!=""){
			$sql = "SELECT doctor_id,hospital_id,count(schedule_id) as total_apointment,sum(duration) as total_duration FROM schedule WHERE (hospital_id = '".$id."' and schedule.doctor_id = '".$doctor."' and type='appointment')  group by doctor_id  ORDER BY created_date DESC";
		}else if($from_date!="" && $to_date!=""){
			$sql ="SELECT doctor_id,hospital_id,count(schedule_id) as total_apointment,sum(duration) as total_duration FROM  schedule		 
		  	WHERE  (hospital_id = '".$id."' and  DATE(schedule.whens) >= '$from_date' and DATE(schedule.whens)  <= '$to_date'  OR (DATE(schedule.whens) = '$from_date') and type='appointment')  group by doctor_id ORDER BY created_date DESC";			 
		}
	   ...
	   if(count($searchdetail)>0){
		 foreach ($searchdetail as $value) { 
		 	$sqlQ = " select user.user_id,firstname,lastname,hospital_id from user where  hospital_id='".$value->hospital_id."' and status='1'  and user_role='2' and user_id=".$value->doctor_id;
		 	$Pquery = $this->db->query($sqlQ);
	 		$doctordetail =  $Pquery->row();	
	 		 		
			if($doctor!="" && $from_date!="" && $to_date!="" ){
				$sql_b = "SELECT doctor_id,hospital_id,duration FROM schedule WHERE (hospital_id = '".$value->hospital_id."' and  schedule.doctor_id = '".$value->doctor_id."' and DATE(schedule.whens) >= '$from_date' and DATE(schedule.whens)  <= '$to_date' and type='block')   group by doctor_id  ORDER BY created_date DESC";
				$query_b = $this->db->query($sql_b);
				$blockdetail =  $query_b->row();
			}else if($doctor!=""){
				$sql_b = "SELECT doctor_id,hospital_id,duration FROM schedule WHERE hospital_id = '".$value->hospital_id."' and schedule.doctor_id = '".$value->doctor_id."' and type='block'  group by doctor_id  ORDER BY created_date DESC";
				$query_b = $this->db->query($sql_b);
				$blockdetail =  $query_b->row();
			}else if($from_date!="" && $to_date!=""){
				$sql_b = "SELECT doctor_id,hospital_id,duration FROM schedule WHERE (hospital_id = '".$value->hospital_id."' and  schedule.doctor_id = '".$value->doctor_id."' and DATE(schedule.whens) >= '$from_date' and DATE(schedule.whens)  <= '$to_date' and type='block')   group by doctor_id  ORDER BY created_date DESC";
			 ...
			 $msg.='<span class="text-primary">'.$doctordetail->firstname."  ".$doctordetail->lastname.'</span>';
			 $msg.='</div></div></td>';

			 $msg.='<td><span class="text-primary">'.$value->total_apointment.'</span></td>';
			 $msg.='<td>'.$value->total_duration.'</td>';
			 $msg.='<td>'.count($blockdetail).'</td>';
			 $msg.='<td>'.$blockdetail->duration.'</td>
			</tr>';
	...
	function productivity_report_count($patient_id=""){
		$id = $this->session->userdata('user_id');
		    $created_by_id = $this->session->userdata('created_by');
        	$isadmin = $this->session->userdata('isadmin');
	        if($isadmin == 1){
		...
		$sql = "";
   		if($doctor!="" && $from_date!="" && $to_date!="" ){
   			$sql ="SELECT doctor_id,count(schedule_id) as total_apointment,sum(duration) as total_duration FROM schedule WHERE  hospital_id = '".$id."' and  schedule.doctor_id = '".$doctor."' and DATE(schedule.whens) >= '$from_date' and DATE(schedule.whens)  <= '$to_date' and type='appointment'  group by doctor_id";

			
		}else if($doctor!=""){
			$sql ="SELECT doctor_id,count(schedule_id) as total_apointment,sum(duration) as total_duration FROM schedule WHERE hospital_id = '".$id."' and schedule.doctor_id = '".$doctor."' and type='appointment'  group by doctor_id";
		}else if($from_date!="" && $to_date!=""){
			 $sql ="SELECT doctor_id,count(schedule_id) as total_apointment,sum(duration) as total_duration FROM  schedule		 
		  	WHERE  hospital_id = '".$id."' and  DATE(schedule.whens) >= '$from_date' and DATE(schedule.whens)  <= '$to_date'  OR (DATE(schedule.whens) = '$from_date') and type='appointment'  group by doctor_id";
		}
		$query = $this->db->query($sql);
		$searchdetail =  $query->row();
		$msg ='';
		if(!empty($searchdetail)){
			 if($searchdetail->total_duration >=60){
			 	$hours = floor($searchdetail->total_duration / 60).':'.($searchdetail->total_duration -   floor($searchdetail->total_duration / 60) * 60)." <h5 class='text-primary' style='padding-left:100px;'>Hours</h5>";
			 }else{
			 	$hours = $searchdetail->total_duration." <h5 class='text-primary' style='padding-left:100px;'>Minutes</h5>";
			 }

		   	 $msg.='<div class="col-lg-6" style="border: 1px solid rgba(0, 0, 0, 0.125);min-height:100px;">
	          <span class="text-primary fa fa-calendar pull-left" style="font-size:75px;padding-top:10px;" aria-hidden="true"></span>
	          <h3 style="font-size:75px;" class="text-primary">'.$searchdetail->total_apointment.'</h3>
	          <h5 class="text-primary" style="padding-left:100px;">Appointment</h5>
	        </div>
	        <div class="col-lg-5" style="border: 1px solid rgba(0, 0, 0, 0.125);min-height:100px;margin-left: 15px;">
	          ...
	          <h3 style="font-size:75px;" class="text-primary">'.$hours.'</h3>

	        </div>';
    	}else{
	        $msg.='<div class="col-lg-6" style="border: 1px solid rgba(0, 0, 0, 0.125);min-height:100px;">
	          <span class="text-primary fa fa-calendar pull-left" style="font-size:75px;padding-top:10px;" aria-hidden="true"></span>
	          <h2 style="font-size:75px;" class="text-primary">0</h2>
	        </div>
	        <div class="col-lg-5" style="border: 1px solid rgba(0, 0, 0, 0.125);min-height:100px;margin-left: 15px;">
	          <span class="text-primary fa fa-clock-o pull-left" style="font-size:75px;padding-top:10px;" aria-hidden="true"></span>
	          <h2 style="font-size:75px;" class="text-primary">0</h2>
	...
	public function download_excel()
	{
		...
		$objPHPExcel->getActiveSheet()->setCellValue('A1', 'Productivity Report');
		...
		$objPHPExcel->getActiveSheet()->setCellValue('A2', 'Doctor');
		$objPHPExcel->getActiveSheet()->setCellValue('B2', 'Appointment');
		$objPHPExcel->getActiveSheet()->setCellValue('C2', 'Appointment Duration');
		$objPHPExcel->getActiveSheet()->setCellValue('D2', 'Breaks');
		$objPHPExcel->getActiveSheet()->setCellValue('E2', 'Break Duration');
	    ...
	    if((count($this->uri->segment_array()) == 5 && $this->uri->segment(3) !="" ) || count($this->uri->segment_array()) == 3){
			$doctor 	= ($this->uri->segment(3) !="") ? $this->uri->segment(3) : "";
		...
		$doctors = str_replace("%20", " ", $doctor);

		$sql = "";
	   		if($doctor!="" && $from_date!="" && $to_date!="" ){
	   			$sql ="SELECT doctor_id,count(schedule_id) as total_apointment,sum(duration) as total_duration FROM schedule WHERE  hospital_id = '".$id."' and  schedule.doctor_id = '".$doctor."' and DATE(schedule.whens) >= '$from_date' and DATE(schedule.whens)  <= '$to_date'  group by doctor_id  ORDER BY created_date DESC";

				
			}else if($doctor!=""){
				$sql ="SELECT doctor_id,count(schedule_id) as total_apointment,sum(duration) as total_duration FROM schedule WHERE hospital_id = '".$id."' and schedule.doctor_id = '".$doctor."'  group by doctor_id  ORDER BY created_date DESC";
			}else if($from_date!="" && $to_date!=""){
				$sql ="SELECT doctor_id,count(schedule_id) as total_apointment,sum(duration) as total_duration FROM  schedule		 
			  	WHERE  hospital_id = '".$id."' and  DATE(schedule.whens) >= '$from_date' and DATE(schedule.whens)  <= '$to_date'  OR (DATE(schedule.whens) = '$from_date')  group by doctor_id ORDER BY created_date DESC";
			}
		   		
			$query = $this->db->query($sql);
			$productivity_report = $query->result();
			
			foreach ($productivity_report as $value) { 
				$sqlQ = " select user.user_id,firstname,lastname from user where status='1' and role_id='2' and user_id=".$value->doctor_id;
		 		$Pquery = $this->db->query($sqlQ);
	 			$doctordetail =  $Pquery->row();

			 	$objPHPExcel->getActiveSheet()->setCellValue('A'.$row, $patientdetail->prefix.' '.$doctordetail->firstname."  ".$doctordetail->lastname);
				$objPHPExcel->getActiveSheet()->setCellValue('B'.$row, $value->total_apointment);
				$objPHPExcel->getActiveSheet()->setCellValue('C'.$row, $value->total_duration);
				$objPHPExcel->getActiveSheet()->setCellValue('D'.$row, $value->total_duration);
				$objPHPExcel->getActiveSheet()->setCellValue('E'.$row, $value->total_duration);
			...
			header('Content-Disposition: attachment;filename="Productivity Report.xls"');
...
'reminder_call' 	   => $this->input->post('reminder_call'),
	...
	function productivity_report_pdf()
	{
		...
		$pdfname = "Productivity_report" . date('YmdHis') . '.pdf';
		$html = '<style>
								table {
            			...
            			<h3 style="text-align:center;">Productivity Report</h3>
			...
			foreach ($productivity_report as $value) { 
				$sqlQ = " select user.user_id,firstname,lastname from user where status='1' and role_id='2' and user_id=".$value->doctor_id;
				 ...
				 $html.='<span class="text-primary">'.$doctordetail->firstname."  ".$doctordetail->lastname.'</span>';
				 $html.='</div></div></td>';

				 $html.='<td><span class="text-primary">'.$value->total_apointment.'</span></td>';
				 $html.='<td>'.$value->total_duration.'</td>';
				 $html.='<td>'.$value->total_duration.'</td>';
				 $html.='<td>'.$value->total_duration.'</td>
				</tr>';
