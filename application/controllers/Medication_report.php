...
class Medication_report extends CI_Controller {
	...
	public function index()
	{
		...
		$drug_name  = $this->db->get_where("drug",array("status"=>1))->result();
		$data['drug_name'] = $drug_name;   
		$data['title'] = display('Medication Report');
		$data['patients'] = $this->patient_model->read();
		$data['content'] = $this->load->view('medication_report',$data,true);
		$this->load->view('layout/main_wrapper',$data);
	}
	
	
	function medication_report_search($patient_id=""){
		...
		$sql = "";
	   		if($medication!="" && $from_date!="" && $to_date!="" ){
				$sql ="SELECT * FROM e_prescription		 
			  	WHERE  e_prescription.drug_name = '".$medication."' and DATE(e_prescription.datetime_prescribed) >= '$from_date' and DATE(e_prescription.datetime_prescribed)  <= '$to_date' ORDER BY date DESC";
			}elseif($medication!=""){
				$sql ="SELECT * FROM e_prescription		 
			  	WHERE  e_prescription.drug_name = '".$medication."'  ORDER BY date DESC";
			}elseif($from_date!="" && $to_date!=""){
				$sql ="SELECT * FROM e_prescription		 
			  	WHERE   DATE(e_prescription.datetime_prescribed) >= '$from_date' and DATE(e_prescription.datetime_prescribed)  <= '$to_date' OR (DATE(e_prescription.datetime_prescribed) = '$from_date') ORDER BY date DESC";
			}
...
/* 
					 ...
					 $msg.='<td><div class="kpull-left"><div class="word-break">';
					 $msg.='<span class="text-primary">'.$value->drug_name .'</span>';
					 $msg.='</div></div></td>';
					 $msg.='<td>'.$value->order_status.'</td>';

					 $msg.='<td>'.date("d-m-Y",strtotime($value->datetime_prescribed)).'</td>';
					 $msg.='<td>'.date("d-m-Y",strtotime($value->datetime_started_taking)).'</td>';
					 $msg.='<td>'.date("d-m-Y",strtotime($value->datetime_stopped_taking)).'</td>
					</tr>';
	...
	public function sendEmailAttachment($to, $subject, $htmlMessage) {
		...
		$objPHPExcel->getActiveSheet()->setCellValue('A1', 'Medication Report');
		...
		$objPHPExcel->getActiveSheet()->setCellValue('C2', 'Medication');
		$objPHPExcel->getActiveSheet()->setCellValue('D2', 'Status');		
		$objPHPExcel->getActiveSheet()->setCellValue('F2', 'Prescribed');
		$objPHPExcel->getActiveSheet()->setCellValue('G2', 'Start Taking');
		$objPHPExcel->getActiveSheet()->setCellValue('H2', 'End Taking');
		...
		if((count($this->uri->segment_array()) == 5 && $this->uri->segment(3) !="" ) || count($this->uri->segment_array()) == 3){
			$medication 	= ($this->uri->segment(3) !="") ? $this->uri->segment(3) : "";
		...
		$medications = str_replace("%20", " ", $medication);
		$sql = "";
		if($medications!="" && $from_date!="" && $to_date!="" ){
			$sql = "SELECT * FROM e_prescription		 
			  	WHERE  e_prescription.drug_name = '".$medications."' and DATE(e_prescription.datetime_prescribed) >= '$from_date' and DATE(e_prescription.datetime_prescribed)  <= '$to_date' ORDER BY date DESC";
		}else if($medications!=""){
			$sql ="SELECT * FROM e_prescription WHERE  e_prescription.drug_name = '".$medications."'  ORDER BY date DESC";
		}else if($from_date!="" && $to_date!=""){
			$sql ="SELECT * FROM e_prescription		 
			  	WHERE   DATE(e_prescription.datetime_prescribed) >= '$from_date' and DATE(e_prescription.datetime_prescribed)  <= '$to_date' OR (DATE(e_prescription.datetime_prescribed) >= '$from_date') ORDER BY date DESC";
		}		
		$query = $this->db->query($sql);
		$medication_report = $query->result();
		
		if(count($medication_report)>0){
			foreach ($medication_report as $value) { 
				$sqlQ = " select patient.id,fname,lname,prefix,date_of_birth from patient where id=".$value->patient_id;
			 	$Pquery = $this->db->query($sqlQ);
		 		$patientdetail =  $Pquery->row();
				...
				$objPHPExcel->getActiveSheet()->setCellValue('C'.$row, $value->drug_name);
				$objPHPExcel->getActiveSheet()->setCellValue('D'.$row, $value->order_status);				
				$objPHPExcel->getActiveSheet()->setCellValue('F'.$row, $value->datetime_prescribed);								
				$objPHPExcel->getActiveSheet()->setCellValue('G'.$row, $value->datetime_started_taking);								
				$objPHPExcel->getActiveSheet()->setCellValue('H'.$row, $value->datetime_stopped_taking);								
				
				$row++;
			}
			
			header('Content-Type: application/vnd.ms-excel');
			header('Content-Disposition: attachment;filename="Medication_Report.xls"');
...
redirect('patient');
	...
	function medication_report_pdf()
	{
		//echo count($this->uri->segment_array());exit;
		if((count($this->uri->segment_array()) == 5 && $this->uri->segment(3) !="" ) || count($this->uri->segment_array()) == 3){
			$medication 	= ($this->uri->segment(3) !="") ? $this->uri->segment(3) : "";
		...
		$pdfname = "Medication_report" . date('YmdHis') . '.pdf';
		$html = '<style>
								table {
            			...
            			<h3 style="text-align:center;">Medication Report</h3>
			...
			$query = $this->db->query($sql);
			$medication_report = $query->result_array();
			//print_r($medication_report);exit;
			foreach ($medication_report as $medication_reports) { 
				$sqlQ = " select patient.id,fname,lname,prefix,date_of_birth from patient where id=".$medication_reports['patient_id'];
		 		$Pquery = $this->db->query($sqlQ);
	 			$patientdetail =  $Pquery->row_array();
					...
					$html.='<tr style="border-bottom: 1px solid #ddd;" class="hovertr">';
					 ...
					 $html.='<td><div class="kpull-left"><div class="word-break">';
					 $html.='<span class="text-primary">'.$medication_reports['drug_name'].'</span>';
					 $html.='</div></div></td>';

					 $html.='<td>'.$medication_reports['order_status'].'</td>';
					 $html.='<td>'.date("d-m-Y h:i:s",strtotime($medication_reports['datetime_prescribed'])).'</td>';
					 $html.='<td>'.date("d-m-Y",strtotime($medication_reports['datetime_started_taking'])).'</td>';
					 $html.='<td>'.date("d-m-Y",strtotime($medication_reports['datetime_stopped_taking'])).'</td>
					</tr>';
