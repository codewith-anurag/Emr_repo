...
class Patient extends CI_Controller {
	...
	public function index($id = null)
	{
		...
		$data['patients'] = $this->patient_model->read($id);
...
//		exit;
	...
	$patients =	$this->patient_model->read($id);
	...
	$objWriter->save('php://output');
		...
		$sql ="SELECT * FROM user WHERE (user_role='2' AND created_by = '".$doctor_id."' ) and (firstname like '%".($p_id)."%' or email like '%".($p_id)."%' or lastname like '%".($p_id)."%' or date_of_birth like '%".($p_id)."%') ORDER BY user_id DESC";
		...
		foreach ($searchdetail as $value) {
			...
			$imagePath = $value->picture ? $value->picture:'assets/images/patient/2017-01-16/p5.png';

			$value->date_of_birth=date('d/m/Y',strtotime($value->date_of_birth));
			$msg.='<tr style="border-bottom: 1px solid #ddd;" class="hovertr"><td>';
			$msg.='<img style="width: 50px;" src="'.base_url().$imagePath.'"></td>';
			$msg.='<td onclick="doctor_info('.$value->user_id.')"><div class="kpull-left"><div class="word-break">';
			...
			$msg.='</div></div></td>';
			$msg.='<td class="text-primary" onclick="doctor_info('.$value->user_id.')"><span class="text-primary">'.$value->lastname.'</span></td>';
			$msg.='<td class="text-primary" onclick="doctor_info('.$value->user_id.')">'.$value->date_of_birth.'<br>'.$value->sex.'</td>';
			$msg.='<td class="text-primary" onclick="doctor_info('.$value->user_id.')">'.$value->address.'<br><span class="light-grey">M</span>'.$value->mobile.'<span class="light-grey ml-15">H</span>'.$value->phone.'</td>';
			$msg.= '<td class="text-primary" onclick="doctor_info('.$value->user_id.')">';
				$role = $this->db->select("*")->from("role")->where("r_id",$value->role_id)->get()->row();
				if(isset($role)){
					$msg.=$role->name;
				}
			$msg.='</td>';
			$msg.= '<td><div class="btn-group"><select class="btn btn-default form-control" onchange="call(\''.$value->user_id.'\',this.options[this.selectedIndex].value,'.$doctor_id.')"><option value="1" '.(($value->status==1)?'Selected':'').'>Active</option><option value="0"'.(($value->status==0)?'Selected':'').'>Inactive</option>   </select>
			</div></td>';
			$msg.='<td class="pt-15"><div class="btn-group" style="float: right;display: flex;">
			<a href="#" onclick="doctor_info('.$value->user_id.')" class="btn btn-xs btn-default" style="margin-right:10px;"><i class="fa fa-eye"></i></a><a href="'.base_url("dashboard_super/doctor/doctor/edit/$doctor_id/$value->user_id").'" class="btn btn-xs btn-default" style="margin-right:10px;"><i class="fa fa-edit"></i></a>';
			$msg.='<a href="'.base_url("dashboard_super/doctor/doctor/delete/$doctor_id/$value->user_id").'" class="btn btn-xs btn-danger" onclick="return confirm('.display('are_you_sure').')"><i class="fa fa-trash"></i></a></div>
			</td>
			</tr>';
		}
...
public function changestatus($id= nulll)
		{
     ...
     if($this->input->get_post('ethnicity_race')=='choose'){
			...
			if ($this->input->post('id') == null) { //create a patient
				$data['patient'] = (object)$postData = [
					...
					'date_of_birth' => date('Y-m-d', strtotime(($this->input->post('date_of_birth') != null)? $this->input->post('date_of_birth'): "0000-00-00")),
...
'reminder_call' 	   => $this->input->post('reminder_call'),
				...
				//	'created_by'   => $this->session->userdata('user_id'),
					'status'       => '1',
				//	'hospital_id'   => $admin_id,//$this->session->userdata('user_id'),
...
'reminder_call' 	   => $this->input->post('reminder_call'),
		...
		redirect('dashboard_super/patient/patient/'.$patient_id.'/'.$admin_id);
				...
				// print_r($postData);
				 //exit;
					if ($this->patient_model->update($postData)) {
						...
						#set exception message
						$this->session->set_flashdata('exception', display('please_try_again'));
						redirect('dashboard_super/patient/patient/'.$postData['id']);
					}
...
redirect('patient');
	...
	function patient_report($id=null)
	{
...
$this->dompdf->load_html($html);
	...
	{
			$data['admin_id']=$this->uri->segment(6);
			//exit;
	 	$patient_id = $this->uri->segment(5);
