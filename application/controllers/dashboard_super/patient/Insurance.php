...
class Insurance extends CI_Controller {
	...
	public function __construct()
	{
      ...
      'dashboard_super/patient/insurance_model',
			'dashboard_super/patient/patient_model',
			'dashboard_super/patient/doctor_model',
...
redirect('insurance');
    ...
    */
	...
	$objPHPExcel->getActiveSheet()->setCellValue('A2', 'Payer');
	$objPHPExcel->getActiveSheet()->setCellValue('B2', 'Patient Id');
	$objPHPExcel->getActiveSheet()->setCellValue('C2', 'Type');
	$objPHPExcel->getActiveSheet()->setCellValue('D2', 'Insurance Id');
	$objPHPExcel->getActiveSheet()->setCellValue('E2', 'Effective From');
	$objPHPExcel->getActiveSheet()->setCellValue('F2', 'Effective To');
	$objPHPExcel->getActiveSheet()->setCellValue('G2', 'Copay');
	$objPHPExcel->getActiveSheet()->setCellValue('H2', 'Eligibility');
	...
	$patient_id = $this->db->select("*")->from("patient")->where("hospital_id",$id)->get()->row();
	$insurance = $this->db->select("*")->from("insurance")->where("hospital_id",$id)->get()->result();
	...
	//	$value->create_date = date('d/m/Y',strtotime($value->create_date));
			$objPHPExcel->getActiveSheet()->setCellValue('A'.$row, $value->payer_name);
			$objPHPExcel->getActiveSheet()->setCellValue('B'.$row, $value->patient_id);
			$objPHPExcel->getActiveSheet()->setCellValue('C'.$row, $value->plan_name_and_type);
			$objPHPExcel->getActiveSheet()->setCellValue('D'.$row, $value->insurance_id);
			$objPHPExcel->getActiveSheet()->setCellValue('E'.$row, $value->effective_from);
			$objPHPExcel->getActiveSheet()->setCellValue('F'.$row, $value->effective_to);
			$objPHPExcel->getActiveSheet()->setCellValue('G'.$row, $value->copay_type);
			$objPHPExcel->getActiveSheet()->setCellValue('H'.$row, $value->eligiblility);
			$objPHPExcel->getActiveSheet()->setCellValue('I'.$row, $value->status);
			$row++;
	}
